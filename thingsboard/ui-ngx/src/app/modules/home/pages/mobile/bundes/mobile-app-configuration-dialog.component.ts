...
export class MobileAppConfigurationDialogComponent extends DialogComponent<MobileAppConfigurationDialogComponent> {
  ...
  gitRepositoryLink = 'git clone -b master https://github.com/hospicare/flutter_hospicare_app.git';
  ...
  downloadSettings(): void {
    const settings: any = {
      hospicareApiEndpoint: window.location.origin,
    ...
    if (!!this.data.androidApp) {
      ...
      settings.hospicareOAuth2CallbackUrlScheme = this.data.androidApp.pkgName + '.auth';
      settings.hospicareAndroidAppSecret = this.data.androidApp.appSecret;
    }
    if (!!this.data.iosApp) {
      ...
      settings.hospicareIosAppSecret = this.data.iosApp.appSecret;
    }
