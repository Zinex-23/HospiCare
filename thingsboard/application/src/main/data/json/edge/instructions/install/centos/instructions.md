Here is the list of commands that can be used to quickly install HospiCare Edge on RHEL/CentOS 7/8 and connect to the server.
...
HospiCare service is running on Java 17. To install OpenJDK 17, follow these instructions:
...
#### Step 2. Configure HospiCare Edge Database

HospiCare Edge supports **SQL** and **hybrid** database configurations.
In this guide, we’ll use an **SQL** database.
For more details about the hybrid setup, please refer to the official installation instructions on the <a href="https://hospicare.io/docs/user-guide/install/edge/rhel/#step-2-configure-hospicare-database" target="_blank">HospiCare documentation site</a>.
...
Since HospiCare Edge uses the PostgreSQL database for local storage, configuring MD5 authentication ensures that only authenticated users or 
...
and create the HospiCare Edge database (tb_edge). To connect to the PostgreSQL database, enter the PostgreSQL password.
...
#### Step 3. HospiCare Edge Service Installation
Download installation package:

```bash
wget https://github.com/hospicare/hospicare-edge/releases/download/v${TB_EDGE_TAG}/tb-edge-${TB_EDGE_TAG}.rpm
{:copy-code}
```

Go to the download repository and install HospiCare Edge service:
...
#### Step 4. Configure HospiCare Edge
To configure HospiCare Edge, you can use the following command to automatically update the configuration file with specific values:
...
If HospiCare Edge runs on the same machine as the HospiCare Server, you need to update the port configuration to avoid conflicts between the two services.

Please execute the following command to update HospiCare Edge configuration file (**/etc/tb-edge/conf/tb-edge.conf**):
...
Once HospiCare Edge is installed and configured, please execute the following install script:
...
#### Step 6. Start HospiCare Edge Service
...
Log in using your **tenant credentials** from either your local HospiCare Server or the **HospiCare Live Demo**.